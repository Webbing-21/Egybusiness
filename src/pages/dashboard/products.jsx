...
export default function DashboardProducts() {
  ...
  useEffect(() => {
    getAllProducts(`pageNumber=${currentPage}&PRODUCT_PER_PAGE=${10}`).then(data => {
      ...
      setTotalProducts(data?.totalProductCount || 0);
    });
  }, [refresh, currentPage]);

  // دالة لمعالجة بيانات المنتج قبل إرسالها للتعديل
          ...
          {products.map((product) => (
            <tr key={product._id}>
              ...
              <td className="d-flex gap-2">
                <AddProducts 
                  product={prepareProductData(product)} 
                ...
                />
                <DeleteProductButton 
                  handeler={() => deleteProduct(product._id)} 
                  setRefresh={setRefresh} 
